...
import com.mycompany.twitchPlus.entity.User;
...
public class MySQLConnection {
    ...
    public void setFavoriteItem(String userId, Item item) throws MySQLException {
        ...
        try {
            ...
            statement.setString(1, userId);  // index starts from 1, not 0
        ...
        // an alternative way by using string format, %s, but still prefer the upper way
        // String sql = String.format("INSERT INTO favorite_records (user_id, item_id) VALUES (%s, %s)", userId, item.getId())
        // this way cannot avoid malicious operation if someone typed userId as "Vincent or 1 = 1", it will always true and delete all users
    }
    ...
    public void saveItem(Item item) throws MySQLException {
        ...
        String sql = "INSERT IGNORE INTO items VALUES (?, ?, ?, ?, ?, ?, ?)"; // IGNORE will make program skip this operation
        try {                                                                 // if there already exists this in db
            PreparedStatement statement = conn.prepareStatement(sql);
    ...
    public Set<String> getFavoriteItemIds(String userId) throws MySQLException {
        ...
        try {
            PreparedStatement statement = conn.prepareStatement(sql);
            statement.setString(1, userId);
            ResultSet rs = statement.executeQuery();  // ResultSet, JDBC provide, like a 1d hashset
    ...
    public Map<String, List<Item>> getFavoriteItems(String userId) throws MySQLException {
        ...
        try {
            ...
            for (String itemId : favoriteItemIds) {
                ...
                if (rs.next()) {  // only one item in set, so we can use "if" here
    ...
    public boolean addUser(User user) throws MySQLException {
        ...
        try {
            ...
            return statement.executeUpdate() == 1;  // 判断是否等于1的目的是，如果return 0则没改成，否则改了几行返回几
                                                    // 如果等于1说明插入成功，不可能等于2, 3...因为就插入了一行，也可能等于0说明没加进去
                                                    // 发生了user id duplicate, 因为上面sql syntax我们加了IGNORE所以不会抛exception
                                                    // 上面加IGNORE原因: 方便找问题，是duplicate还是server没连上，否则都抛exception不好判断
        } catch (SQLException e) {
            e.printStackTrace();

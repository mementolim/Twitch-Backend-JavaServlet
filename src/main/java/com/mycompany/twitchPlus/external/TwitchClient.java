...
public class TwitchClient {
    ...
    private String buildGameURL(String url, String gameName, int limit) {
        if (gameName.equals("")) {
            ...
            // string.format purpose, alternate %s in url with limit

        } else {
            try {
                gameName = URLEncoder.encode(gameName, "UTF-8");
                // why we need encode here?
                // Purpose: encode special characters in URL, e.g. Rick Sun -> Rick%20Sun
    ...
    private String searchTwitch(String url) throws TwitchException, IOException {
        ...
        ResponseHandler<String> responseHandler = response -> {
            // lamda, better way to classic, response is input, can be any name "resp" "res" here
...
//        read config file by using input stream
        ...
        try {
            ...
            return httpclient.execute(request, responseHandler); // this is call-back operation
        } catch (IOException e) {
            e.printStackTrace();
            throw new TwitchException("Failed to get result from Twitch API");
        } finally {
    ...
    private List<Game> getGameList(String data) throws TwitchException {
        ...
        try {
            return Arrays.asList(mapper.readValue(data, Game[].class)); // need a class type so we use .class
    ...
    private List<Item> searchStreams(String gameId, int limit) throws TwitchException, IOException {
        ...
        for (Item item : streams) {
            ...
            item.setUrl(TWITCH_BASE_URL + item.getBroadcasterName()); // no url in json return so we need to set it
        }
        return streams;
    }
